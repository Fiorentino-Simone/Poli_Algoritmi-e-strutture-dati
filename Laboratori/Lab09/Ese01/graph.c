...
{
    int sumWeight, maxWeight, j;
    sumWeight = maxWeight = j = 0;
    ...
    printf("MAX %s %s %d\n", STsearchByIndex(G->tab, backEdges[j].v), STsearchByIndex(G->tab, backEdges[j].w), backEdges[j].wt);

    GRAPHremoveE(G, backEdges[j].v, backEdges[j].w);
    GRAPHstore(G, stdout);
}
...
static void DAGlongestpath(Graph D, int *ts, int id, int i)
{
    ...
    {
        w = ts[j];
        printf("Piu lungo da %s\n", STsearchByIndex(D->tab, w));
        if (d[w] != -1){
