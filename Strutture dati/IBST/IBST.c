...
struct IBSTnode
{
    ...
    link l, r;
    int N;
    int max; // mi porto dietro il massimo valore dei sottoalberi sx e dx
};
...
static link insertR(link h, Item item, link z)
{
    if (h == z)
...
static link deleteR(link h, Item item, link z)
{
    link x;
    if (h == z)
        return z;
